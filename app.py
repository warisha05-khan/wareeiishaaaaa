...
def show_login_page():
    ...
    with col2:
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
            ...
            <h1 style="text-align: center; color: white; margin-bottom: 30px;">🏥 ICT Health</h1>
            <h2 style="text-align: center; color: white; margin-bottom: 30px; font-size: 1.5rem;">Hospital Management System</h2>
        </div>
        """, unsafe_allow_html=True)
        ...
        with tab2:
            ...
            with col2:
                st.markdown("""
                <div style="background: #f0f2f5; padding: 15px; border-radius: 10px; margin-top: 20px;">
                    <small>⚠️ New patient? Register first using the registration form.<br>
...
def show_admin_dashboard():
    ...
    menu = st.sidebar.selectbox(
        ...
        ["📊 Dashboard", "👨‍👩‍👧 Patient Registration", "📊 Vitals Logger", 
         ...
         "📄 Reports", "🤖 AI Health Assistant", "ℹ️ About ICT in Health"]
    )
...
def show_patient_portal():
    ...
    st.sidebar.button("🚪 Logout", on_click=logout, use_container_width=True)
    st.sidebar.markdown(f"---")
    st.sidebar.success(f"✅ Logged in as: **{patient_name}** (Patient)")
